// Exactly the seed data given in the brief, with one deliberate fix:
// pitch:medium's multiplier arrives as the STRING "1.12" in the source
// (every other multiplier is a number). We coerce it to a number here
// AND defensively in the admin PUT route, so a future owner edit can't
// reintroduce the same bug. See DECISIONS.md, "questionable seed data".
  ...
  questions: [
    ...
    },
    {
      ...
      options: [
        ...
        { value: 'medium', label: 'Medium', multiplier: 1.12 }, // coerced from "1.12"
...
// Historical leads from the brief. Two of these (ld_0917) reference a
// config_version and question keys (chimney_count, gutter_replace,
// material:"slate_natural") that don't exist in the current config —
// that's expected and realistic (old data from an earlier config
// version) and is exactly why answers are stored schemaless. We seed
// them as-is for the owner panel's leads list; we do NOT try to force
// them through the current calculator.
const existingLeads = [
  {
...
module.exports = { currentConfig, existingLeads };
